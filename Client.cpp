...
void setUIConnected(bool connected) {
     gdk_threads_add_idle([](gpointer data) -> gboolean {
        bool *conn = static_cast<bool*>(data);
        if (!*conn) {
...
void on_connect_clicked(GtkWidget *widget, gpointer data) {
    ...
    if (inet_pton(AF_INET, ipBuffer.c_str(), &serverAddr.sin_addr) <= 0) {
    ...
    // Connect (blocking)
    if (connect(newSocket, (sockaddr *)&serverAddr, sizeof(serverAddr)) < 0) {
...
void on_send_clicked(GtkWidget *widget, gpointer data) {
    lock_guard<mutex> lock(dataMutex);
    if (!isConnected || clientSocket < 0) return;
